...
def plot_dataset_revenue(prices, demands, save_path='reports/dataset_revenue.png'):
    ...
    """
    print(f"Plotting dataset revenue and saving to {save_path}...")
...
def plot_comparison_all_three(prices, rev_concave, rev_nonconvex, rev_restored,
    optimal_price, optimal_revenue, save_path='reports/revenue_comparison.png'):
    """
    Plots a comparison of concave, non-convex, and restored revenue models.
    """
    print(f"Plotting comparison of all three revenue models and saving to {save_path}...")
    fig, ax = plt.subplots(figsize=(12, 7))
    
    ax.plot(prices, rev_concave, label='Original Concave Revenue', linewidth=2, alpha=0.7)
    ax.plot(prices, rev_nonconvex, '--', label='Non-Convex Revenue', linewidth=2, alpha=0.7)
    ax.plot(prices, rev_restored, ':', label='Restored Concave Revenue', linewidth=5, alpha=0.7)
    ...
    ax.set_xlabel('Price')
    ax.set_ylabel('Revenue')
    ax.set_title('Price vs Revenue — Concave vs Non-Convex vs Restored')
    ax.legend()
    _save_plot(fig, save_path)
    print(f'Saved {save_path}')
...
def plot_separate_revenues(prices, rev_concave, rev_nonconvex, rev_restored, optimal_price, optimal_revenue, save_dir='reports'):
    ...
    """
    print("Plotting separate revenue models...")
    ...
    print(f"Saved concave revenue plot to {os.path.join(save_dir, 'concave_revenue.png')}")
    ...
    print(f"Saved non-convex revenue plot to {os.path.join(save_dir, 'nonconvex_revenue.png')}")
    ...
    print(f"Saved restored concave revenue plot to {os.path.join(save_dir, 'restored_concave_revenue.png')}")
...
def plot_dataset_convexity(prices, demands, save_path='reports/dataset_convexity.png'):
    ...
    """
    print(f"Checking and plotting dataset convexity and saving to {save_path}...")
...
def plot_make_dataset_convex(prices, demands, save_path='reports/made_convex_dataset.png'):
    ...
    """
    print(f"Making dataset convex and plotting the result, saving to {save_path}...")
    dataset = Dataset(prices, demands)
    dataset.make_convex()
    dataset.plot_hull(save_path=save_path)
